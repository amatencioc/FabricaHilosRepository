...
public static class TemplateRenderer
{
    ...
    public static string Renderizar(string nombreTemplate, Dictionary<string, string> reemplazos)
    {
        ...
        return AplicarReemplazos(html, reemplazos);
    }

    private static string AplicarReemplazos(string html, Dictionary<string, string> reemplazos)
    {
        var sb = new StringBuilder(html);
        foreach (var (clave, valor) in reemplazos)
            sb.Replace($"{{{{{clave}}}}}", valor ?? string.Empty);
        return sb.ToString();
    }
